...
Change Log:
- v1.7.4
  - Fixed line-item term amount widget key collisions by including option group and line item context
  - Stopped tab rendering from forcing selected_package_index to the last package on every rerun
  - Marked orphan cleanup dirty only when rows are actually removed
  - Added safer display text handling in compact right-panel preview
  - Added a small run-ready launch note for local Streamlit execution

- v1.7.3
  - Added deterministic scalar widget key suffixes for package tabs, tiers, option groups, and term amount editors
  - Added reruns after add/delete actions inside nested tab editors so the UI refreshes cleanly
  - Synced raw JSON immediately after nested add/delete actions before rerun
  - Reduced widget-state bleed after Raw JSON replacement by avoiding object id based keys where practical

- v1.7.2
  - Added deterministic package tab labels with truncation for long names
  - Scoped scalar editor widget keys with the active editor prefix to reduce tab-state bleed
  - Added separate JSON preview search keys for right-panel preview versus full JSON Preview page
  - Reset the editor widget prefix before global package actions
  - Avoided marking Raw JSON dirty when the applied JSON matches the current edited JSON

- v1.7.1
  - Hardened package tab labels by prefixing each tab with its package number
  - Added raw JSON dirty-state tracking when raw JSON changes are applied
  - Added dirty-state tracking when orphan term amounts are auto-removed
  - Added explicit widget keys to contract term action buttons and empty package add button
  - Expanded compact right-panel preview to include package-level option group counts
...
- v1.6.17
  - Added validation to require exactly one default initial contract term
  - Flags missing or multiple isDefault selections in contract initial terms

- v1.6.16
  - Limited card option selectors to Implementations and Enrollments only across template types
  - Left Addons and later groups in the Included Features comparison matrix

- v1.6.15
  - Removed Implementations and Enrollments from the Included Features comparison matrix
  - Kept those groups in the package cards only

- v1.6.14
  - Restored the v1.6.11 card layout for option selectors
  - Limited card radio selectors to option groups through Enrollments
  - Left later groups such as Addons in the Included Features grid only

- v1.6.11
  - Reverted monthly volume selector in preview to a dropdown
  - Added radio button selectors for option groups inside each package card
  - Shows selected option descriptions and term-aware option pricing in preview

- v1.6.9
  - Added the missing clean_preview_text function that render_pricing_page calls
  - Fixed the NameError in the uploaded v1.6.8 file
- v1.6.8
  - Added clean_preview_text helper for preview-safe contract term rendering
  - Preserved line breaks and list items while stripping HTML tags in preview text
  - Fixed NameError in Pricing Page Preview
- v1.6.7
  - Displayed contract term text as normal text in preview instead of rendered markdown blocks
  - Cleaned contract term text so HTML-style content reads naturally
  - Tightened the contract terms preview section layout
- v1.6.6
...
def sync_edited_from_raw_text():
    try:
        ...
        if parsed != st.session_state.get("edited_json"):
            mark_dirty()
...
def scoped_widget_key(name: str) -> str:
    prefix = st.session_state.get("_editor_widget_prefix", "global")
...
def make_package_tab_label(package: Dict[str, Any], index: int, max_chars: int = 28) -> str:
    name = str(package.get("name") or package.get("internalName") or f"Package {index + 1}").strip()
...
def edit_scalar_field(
    ...
    value = obj.get(key)
    suffix = key_suffix or str(id(obj))
    widget_key = scoped_widget_key(f"{label or key}_{suffix}")
    new_value = value

    if isinstance(value, str):
        if len(value) > 120 or "<br" in value or "<strong" in value or "<ul" in value:
            new_value = st.text_area(label or key, value=value, key=widget_key, height=height)
        else:
            new_value = st.text_input(label or key, value=value, key=widget_key)
    elif isinstance(value, bool):
        new_value = st.checkbox(label or key, value=value, key=widget_key)
    elif isinstance(value, int) and not isinstance(value, bool):
        new_value = int(st.number_input(label or key, value=int(value), step=1, key=widget_key))
    elif isinstance(value, float):
        new_value = float(st.number_input(label or key, value=float(value), key=widget_key))
    elif value is None:
        new_value = st.text_input(label or key, value="", key=widget_key)
    else:
        new_value = st.text_input(label or key, value=str(value), key=widget_key)
...
def render_top_level_editor(data: Dict[str, Any]) -> None:
    ...
    for key in keys:
        edit_scalar_field(data, key, key_suffix="top_level")
...
def render_contract_terms_editor(data: Dict[str, Any]) -> None:
    ...
    for key in ["headerDescription", "toolTipText", "toolTipDescription", "isAutoRenewalEnabled"]:
        edit_scalar_field(contract_terms, key, key_suffix="contract_terms")
    ...
    with top_cols[0]:
        if st.button("Add Initial Term", key="contract_add_initial_term"):
            ...
            st.rerun()
    with top_cols[1]:
        if initial_terms and st.button("Delete Last Initial Term", key="contract_delete_last_initial_term"):
            initial_terms.pop()
            mark_dirty()
    ...
    rebuilt_initial = dataframe_to_terms(edited_initial, initial_original)
    if rebuilt_initial != initial_original:
        contract_terms["contractInitialTerm"] = rebuilt_initial
        mark_dirty()
    ...
    rebuilt_auto = dataframe_to_terms(edited_auto, auto_original)
    if rebuilt_auto != auto_original:
        contract_terms["contractAutoRenewalTerm"] = rebuilt_auto
        mark_dirty()
...
def render_package_actions_global(packages: List[Dict[str, Any]]) -> None:
    ...
    with col1:
        if st.button("+ Add Package", use_container_width=True, key=scoped_widget_key("add_package_global")):
            packages.append(default_package())
            normalize_package_sort_orders(packages)
            sync_raw_text_from_edited()
            mark_dirty()
...
def render_package_details_editor(package: Dict[str, Any]) -> None:
    ...
    for key in keys:
        edit_scalar_field(package, key, key_suffix="package")


def render_term_amount_editor(term_amount: Dict[str, Any], context_key: str, ta_idx: int) -> None:
    st.markdown(f"**Term Amount {ta_idx + 1}**")
    suffix = f"term_amount_{context_key}_{ta_idx}"
    cols = st.columns(4)
    with cols[0]:
        edit_scalar_field(term_amount, "initialTermId", key_suffix=suffix)
    with cols[1]:
        edit_scalar_field(term_amount, "amount", key_suffix=suffix)
    with cols[2]:
        if "transactionUnitAmount" in term_amount:
            edit_scalar_field(term_amount, "transactionUnitAmount", key_suffix=suffix)
    with cols[3]:
        if "transactionUnitOverageAmount" in term_amount:
            edit_scalar_field(term_amount, "transactionUnitOverageAmount", key_suffix=suffix)
...
def render_monthly_volumes_editor(package: Dict[str, Any]) -> None:
    ...
    top_cols = st.columns([1, 1, 4])
    with top_cols[0]:
        if st.button("Add Tier", key=scoped_widget_key("add_tier")):
            ...
            st.rerun()
    with top_cols[1]:
        if monthly_volumes and st.button("Delete Last Tier", key=scoped_widget_key("delete_last_tier")):
            monthly_volumes.pop()
            mark_dirty()
            sync_raw_text_from_edited()
            st.rerun()
    ...
    for mv_idx, mv in enumerate(monthly_volumes):
        with st.expander(f"Tier {mv_idx + 1}: {mv.get('internalName', 'Monthly Volume')}", expanded=(mv_idx == 0)):
            cols = st.columns(3)
            with cols[0]:
                if st.button(f"Delete This Tier {mv_idx + 1}", key=scoped_widget_key(f"del_mv_{mv_idx}")):
            ...
            for key in edit_keys:
                edit_scalar_field(mv, key, key_suffix=f"monthly_volume_{mv_idx}")
            ...
            with term_top_cols[0]:
                if st.button("Add Term Amount", key=scoped_widget_key(f"add_ta_{mv_idx}")):
                    term_amounts.append(default_term_amount())
                    mark_dirty()
                    sync_raw_text_from_edited()
                    st.rerun()
            with term_top_cols[1]:
                if term_amounts and st.button("Delete Last Term Amount", key=scoped_widget_key(f"del_last_ta_{mv_idx}")):
                    term_amounts.pop()
                    mark_dirty()
                    sync_raw_text_from_edited()
                    st.rerun()
            ...
            if not term_amounts:
                ...
                for ta_idx, term_amount in enumerate(term_amounts):
                    ...
                    with ta_cols[0]:
                        render_term_amount_editor(term_amount, f"mv_{mv_idx}", ta_idx)
                    with ta_cols[1]:
                        if st.button("Delete", key=scoped_widget_key(f"del_ta_{mv_idx}_{ta_idx}")):
...
def render_option_line_item_editor(line_item: Dict[str, Any], group_idx: int, line_idx: int) -> None:
    ...
    for key in edit_keys:
        edit_scalar_field(line_item, key, key_suffix=f"line_item_{group_idx}_{line_idx}")

    st.markdown("#### Line Item Term Amounts")
    term_amounts = line_item.setdefault("termAmounts", [])
    cols = st.columns([1, 1, 4])
    with cols[0]:
        if st.button("Add Line Item Term Amount", key=scoped_widget_key(f"add_li_ta_{group_idx}_{line_idx}")):
            term_amounts.append(default_term_amount())
            mark_dirty()
            sync_raw_text_from_edited()
            st.rerun()
    with cols[1]:
        if term_amounts and st.button("Delete Last Line Item Term Amount", key=scoped_widget_key(f"del_li_last_ta_{group_idx}_{line_idx}")):
            term_amounts.pop()
            mark_dirty()
            sync_raw_text_from_edited()
            st.rerun()
    ...
    for ta_idx, term_amount in enumerate(term_amounts):
        ...
        ta_cols = st.columns([5, 1])
        with ta_cols[0]:
            render_term_amount_editor(term_amount, f"line_{group_idx}_{line_idx}", ta_idx)
        with ta_cols[1]:
            if st.button("Delete", key=scoped_widget_key(f"del_li_ta_{group_idx}_{line_idx}_{ta_idx}")):
                term_amounts.pop(ta_idx)
                mark_dirty()
                sync_raw_text_from_edited()
                st.rerun()
...
def render_option_groups_editor(package: Dict[str, Any]) -> None:
    ...
    top_cols = st.columns([1, 1, 4])
    with top_cols[0]:
        if st.button("Add Option Group", key=scoped_widget_key("add_option_group")):
            option_groups.append(default_option_group())
            mark_dirty()
            sync_raw_text_from_edited()
            st.rerun()
    with top_cols[1]:
        if option_groups and st.button("Delete Last Group", key=scoped_widget_key("delete_last_group")):
            option_groups.pop()
            mark_dirty()
            sync_raw_text_from_edited()
            st.rerun()
    ...
    for group_idx, group in enumerate(option_groups):
        ...
        with st.expander(f"Group {group_idx + 1}: {title}", expanded=(group_idx == 0)):
            cols = st.columns([1, 5])
            with cols[0]:
                if st.button(f"Delete Group {group_idx + 1}", key=scoped_widget_key(f"delete_group_{group_idx}")):
            ...
            for key in [
                ...
                edit_scalar_field(group, key, key_suffix=f"option_group_{group_idx}")
            ...
            with line_top_cols[0]:
                if st.button("Add Line Item", key=scoped_widget_key(f"add_line_item_{group_idx}")):
                    line_items.append(default_option_line_item())
                    mark_dirty()
                    sync_raw_text_from_edited()
                    st.rerun()
            with line_top_cols[1]:
                if line_items and st.button("Delete Last Line Item", key=scoped_widget_key(f"del_last_line_item_{group_idx}")):
                    line_items.pop()
                    mark_dirty()
                    sync_raw_text_from_edited()
                    st.rerun()
            ...
            if not line_items:
                ...
                for line_idx, line_item in enumerate(line_items):
                    ...
                    with st.expander(f"Line Item {line_idx + 1}: {line_title}", expanded=False):
                        top = st.columns([1, 5])
                        with top[0]:
                            if st.button("Delete Line Item", key=scoped_widget_key(f"del_line_item_{group_idx}_{line_idx}")):
...
def render_raw_json_editor() -> None:
    ...
    with cols[0]:
        if st.button("Apply Raw JSON Changes"):
            before = st.session_state.get("edited_json")
            ok, msg = sync_edited_from_raw_text()
            if ok:
                if st.session_state.get("edited_json") != before:
                    mark_dirty()
                st.success(msg)
            else:
                st.error(msg)
...
def render_validation_panel(data: Dict[str, Any]) -> None:
    ...
    errors.extend(validate_contract_term_defaults(data))
...
def render_json_preview(data: Dict[str, Any], key_prefix: str = "json_preview") -> None:
    tab1, tab2 = st.tabs(["Edited JSON", "Original JSON"])
    with tab1:
        render_json_preview_panel(safe_json_dumps(data), f"{key_prefix}_search_edited")
    with tab2:
        render_json_preview_panel(safe_json_dumps(st.session_state["original_json"]), f"{key_prefix}_search_original")
...
def render_pricing_page_compact(data: Dict[str, Any]) -> None:
    st.markdown("#### Pricing Preview")

    product_name = data.get("name") or "Unnamed product"
    st.markdown(f"**{product_name}**")

    header_text = truncate_preview_text(data.get("headerDescription") or data.get("description") or "", 120)
    if header_text:
    ...
    if packages:
        for package in packages:
            package_name = clean_preview_text(package.get("name") or package.get("internalName") or "Unnamed Package")
            amount = format_money(package.get("amount", ""))
            unit = clean_preview_text(package.get("unit", ""))
            if amount:
                suffix = f"/{unit}" if unit else ""
                st.write(f"{package_name}: {amount}{suffix}")
            else:
                st.write(package_name)
    else:
        st.caption("No packages found.")

    contract_initial_terms = (data.get("contractTerms", {}) or {}).get("contractInitialTerm", []) or []
    if contract_initial_terms:
        labels = build_contract_term_labels(contract_initial_terms)
        default_index = get_default_contract_term_index(contract_initial_terms)
        st.selectbox(
            "Contract Term",
            options=list(range(len(contract_initial_terms))),
            index=default_index,
            format_func=lambda i: labels[i],
            key="compact_contract_term_select",
        )

    option_group_summaries: List[str] = []

    for group in data.get("optionItems", []) or []:
        group_name = group.get("optionItemType") or "Option Group"
        item_count = len(group.get("optionLineItems", []) or [])
        item_label = "item" if item_count == 1 else "items"
        option_group_summaries.append(f"{group_name}: {item_count} {item_label}")

    package_group_counts: Dict[str, int] = {}
    for package in packages:
        for group in package.get("optionItems", []) or []:
            group_name = group.get("optionItemType") or "Option Group"
            package_group_counts[group_name] = package_group_counts.get(group_name, 0) + len(group.get("optionLineItems", []) or [])

    for group_name, item_count in package_group_counts.items():
        item_label = "item" if item_count == 1 else "items"
        option_group_summaries.append(f"{group_name}: {item_count} package-level {item_label}")

    if option_group_summaries:
        st.markdown("**Option Groups**")
        for summary in option_group_summaries:
            st.write(summary)

    payment_schedule = truncate_preview_text(data.get("paymentSchedule", ""), 100)
    if payment_schedule:
        st.markdown("**Payment Schedule**")
        st.caption(payment_schedule)
...
def render_right_panel(data: Dict[str, Any]) -> None:
    ...
    with tab_json:
        render_json_preview(data, key_prefix="right_panel_json_preview")


def render_status_header(data: Dict[str, Any]) -> None:
    original_filename = st.session_state.get("original_filename", "—")
    pkg_count = len(data.get("packages", []))

    all_errors: List[str] = []
    all_errors.extend(validate_json_structure(data))
    all_errors.extend(validate_contract_term_defaults(data))
    all_errors.extend(validate_orphan_term_references(data))
...
def get_display_amount_for_option_line_item(
    line_item: Dict[str, Any],
    selected_term: Optional[Dict[str, Any]],
    selected_term_idx: int,
) -> Any:
    matched_term_amount = resolve_term_amount_for_volume(line_item, selected_term, selected_term_idx)
    if matched_term_amount:
        return matched_term_amount.get("amount", line_item.get("amount", ""))
    return line_item.get("amount", "")


def build_option_line_item_label(
    line_item: Dict[str, Any],
    selected_term: Optional[Dict[str, Any]],
    selected_term_idx: int,
) -> str:
    label = (
        line_item.get("marketingTitle")
        or line_item.get("internalName")
        or line_item.get("shortDescription")
        or "Option"
    )
    amount = get_display_amount_for_option_line_item(line_item, selected_term, selected_term_idx)
    if amount not in ("", None, 0, 0.0):
        label = f"{label} ({format_money(amount)})"
    return str(label)


def render_package_option_group_selectors(
    package: Dict[str, Any],
    package_idx: int,
    selected_term: Optional[Dict[str, Any]],
    selected_term_idx: int,
) -> None:
    option_groups = package.get("optionItems", []) or []
    allowed_card_groups = {"implementations", "enrollments"}

    for group_idx, group in enumerate(option_groups):
        group_name = (group.get("optionItemType", "") or "").strip() or f"Option Group {group_idx + 1}"
        normalized_group_name = re.sub(r"[^a-z0-9]+", "", group_name.lower())

        if normalized_group_name not in allowed_card_groups:
            continue

        line_items = group.get("optionLineItems", []) or []
        if not line_items:
            if normalized_group_name == "enrollments":
                break
            continue

        st.markdown('<div class="optum-divider"></div>', unsafe_allow_html=True)
        st.markdown(f'<div class="optum-section-label">{html_escape(group_name)}</div>', unsafe_allow_html=True)

        selected_option_idx = st.radio(
            group_name,
            options=list(range(len(line_items))),
            index=0,
            format_func=lambda i, items=line_items: build_option_line_item_label(items[i], selected_term, selected_term_idx),
            key=f"preview_option_group_{package_idx}_{group_idx}",
            label_visibility="collapsed",
        )

        selected_item = line_items[selected_option_idx]
        description = clean_preview_text(
            selected_item.get("longDescription")
            or selected_item.get("shortDescription")
            or selected_item.get("toolTipDescription")
            or ""
        )
        if description:
...
def render_pricing_page(data: Dict[str, Any]) -> None:
    ...
    for idx, package in enumerate(packages):
        with cols[idx]:
            ...
            with border_container:
                ...
                render_package_option_group_selectors(package, idx, selected_term, selected_term_idx)
    ...
    if contract_header or contract_initial_terms:
        ...
        cleaned_contract_header = clean_preview_text(contract_header)
        if cleaned_contract_header:
            with st.container():
                st.write(cleaned_contract_header)

        if contract_initial_terms:
            st.markdown('<div class="optum-term-block">', unsafe_allow_html=True)
            st.markdown('<div class="optum-term-select-label">Select contract term</div>', unsafe_allow_html=True)
            ...
            selected_term_description = clean_preview_text(selected_term.get("description", ""))
            if selected_term_description:
                st.write(selected_term_description)

    st.markdown('<div class="optum-section-title">Included Features</div>', unsafe_allow_html=True)

    excluded_feature_groups = {"implementations", "enrollments"}
    all_group_names = []
    for package in packages:
        for group in package.get("optionItems", []):
            group_name = group.get("optionItemType", "").strip() or "Modules"
            if group_name.strip().lower() in excluded_feature_groups:
                continue
...
def main() -> None:
    ...
    with st.sidebar:
        ...
        if st.session_state.get("load_complete"):
            ...
            all_errors.extend(validate_contract_term_defaults(st.session_state["edited_json"]))
    ...
    clamp_selected_package_index()
    render_status_header(edited_json)

    editor_pages_with_right_panel = [
        "Product Info",
        "Contract Terms",
        "Package Details",
        "Monthly Volume Tiers",
    ...
    if page in editor_pages_with_right_panel:
        ...
        with col_editor:
            if page == "Product Info":
                ...
                if not packages:
                    st.info("No packages found.")
                    st.subheader("Package Actions")
                    if st.button("Add New Package", key="empty_add_new_package"):
                        packages.append(default_package())
                        normalize_package_sort_orders(packages)
                        st.session_state["selected_package_index"] = 0
                        sync_raw_text_from_edited()
                    ...
                    tab_labels = [make_package_tab_label(p, i) for i, p in enumerate(packages)]
        ...
        render_json_preview(edited_json, key_prefix="full_page_json_preview")
