...
- v1.1
  - Reworked app to feel more like the pricing page structure
  - Added page-oriented navigation
  - Added full editing for package monthly volumes
  - Added full editing for option groups and option line items
  - Added add/delete controls for monthly volumes, option groups, and line items
  - Added package-specific editing screens
- v1.2
  - Attempted preview-only pricing page version
- v1.3
  - Restored full editing from v1.1
  - Added live pricing page preview as a separate section
  - Added simple package cards and included feature rendering
"""
...
def render_pricing_page(data: Dict[str, Any]) -> None:
    ...
    for idx, package in enumerate(packages):
        with package_cols[idx]:
            ...
            if marketing_title:
                st.write(marketing_title)
            if tagline:
                st.caption(tagline)

            if package_amount not in ("", None, 0):
                if package_amount_description:
                    st.markdown(f"#### ${package_amount}")
                    st.caption(str(package_amount_description))
                else:
                    st.markdown(f"#### ${package_amount}")

            st.markdown("##### Monthly Volume Tiers")
            if volumes:
                for mv in volumes:
                    min_v = mv.get("minimumValue", "")
                    max_v = mv.get("maximumValue", "")
                    amt = mv.get("amount", 0)
                    amt_desc = mv.get("amountDescription", "")
                    tier_name = mv.get("internalName", "")

                    if tier_name:
                        st.write(tier_name)
                    st.write(f"{min_v} to {max_v}: ${amt}")
                    if amt_desc:
                        st.caption(str(amt_desc))
            else:
                st.write("No monthly volume tiers")
            ...
            if included_features:
                ...
                st.write("No included features flagged")
