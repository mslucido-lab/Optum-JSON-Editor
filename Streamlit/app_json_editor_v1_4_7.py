...
Change Log:
- v1.4.6
  - Replaced monthly volume tier list on preview with a dropdown
  - Selected tier now drives the displayed package price on the preview cards
...
def render_pricing_page(data: Dict[str, Any]) -> None:
    ...
    if contract_terms.get("headerDescription"):
        st.markdown(
            f'<div class="optum-note">{contract_terms.get("headerDescription")}</div>',
            unsafe_allow_html=True,
        )
