...
Change Log:
- v1.6.18
  - Added collapsible tree view to JSON Preview
  - Added expand and collapse controls for child nodes while keeping code view and search
...
def render_json_tree_node(node: Any, path: str, depth: int, expanded_depth: int) -> None:
    if isinstance(node, dict):
        ...
        for key, value in node.items():
            ...
            if isinstance(value, (dict, list)):
                ...
                type_label = "object" if isinstance(value, dict) else "array"
                label = f"{key} ({type_label}, {count})"
                with st.expander(label, expanded=(depth < expanded_depth)):
                    render_json_tree_node(value, child_path, depth + 1, expanded_depth)
            else:
    ...
    if isinstance(node, list):
        ...
        for idx, item in enumerate(node):
            ...
            if isinstance(item, (dict, list)):
                count = len(item) if hasattr(item, "__len__") else 0
                type_label = "object" if isinstance(item, dict) else "array"
                label = f"[{idx}] ({type_label}, {count})"
                with st.expander(label, expanded=(depth < expanded_depth)):
                    render_json_tree_node(item, child_path, depth + 1, expanded_depth)
            else:
                st.write(f"[{idx}]: {item}")
        return
...
def render_json_preview_panel(data: Dict[str, Any], json_text: str, key_prefix: str) -> None:
    search_text = st.text_input("Search JSON", value="", key=f"{key_prefix}_search", placeholder="Search keys, values, IDs")
    ...
    view_mode = st.radio(
        "Preview mode",
        options=["Code view", "Collapsible tree"],
        horizontal=True,
    ...
    if view_mode == "Code view":
        ...
        with controls[0]:
            if st.button("Expand all", key=f"{key_prefix}_expand_all"):
                st.session_state[f"{key_prefix}_expanded_depth"] = 99
        with controls[1]:
            if st.button("Collapse all", key=f"{key_prefix}_collapse_all"):
                st.session_state[f"{key_prefix}_expanded_depth"] = 0

        expanded_depth = int(
            st.session_state.get(f"{key_prefix}_expanded_depth", 1)
        )
        expanded_depth = st.slider(
            "Default expanded depth",
            min_value=0,
            max_value=8,
            value=min(expanded_depth, 8),
            key=f"{key_prefix}_expanded_depth_slider",
        )
        st.session_state[f"{key_prefix}_expanded_depth"] = expanded_depth

        if search_text.strip() and matches:
            st.caption("Tree view stays collapsible. Search results are listed above.")

        render_json_tree_node(data, "", 0, expanded_depth)


def render_json_preview(data: Dict[str, Any]) -> None:
    tab1, tab2 = st.tabs(["Edited JSON", "Original JSON"])
    with tab1:
        render_json_preview_panel(data, safe_json_dumps(data), "json_preview_edited")
    with tab2:
        render_json_preview_panel(st.session_state["original_json"], safe_json_dumps(st.session_state["original_json"]), "json_preview_original")
