...
Change Log:
- v1.6.12
  - Removed add-on selectors from package cards in the pricing preview
  - Added a separate Add-ons section below the package cards
  - Kept monthly volume selection inside the cards
...
def render_pricing_page(data: Dict[str, Any]) -> None:
    ...
    has_any_addons = any(
        (group.get("optionLineItems", []) or [])
        for package in packages
        for group in package.get("optionItems", []) or []
    )

    if has_any_addons:
        st.markdown('<div class="optum-section-title">Add-ons</div>', unsafe_allow_html=True)
        for package_idx, package in enumerate(packages):
            option_groups = package.get("optionItems", []) or []
            package_groups = [group for group in option_groups if group.get("optionLineItems", [])]
            if not package_groups:
                continue

            st.markdown(f'<div class="optum-feature-package-title">{html_escape(package.get("name", "Package"))}</div>', unsafe_allow_html=True)
            ...
            for group_idx, group in enumerate(package_groups):
                ...
                selected_option_idx = st.radio(
                    ...
                    index=0,
                    format_func=lambda i, items=line_items: build_option_line_item_label(items[i], selected_term, selected_term_idx),
                    key=f"preview_addon_group_{package_idx}_{group_idx}",
                    label_visibility="collapsed",
                )
