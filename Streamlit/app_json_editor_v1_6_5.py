...
- v1.6.3
  - Introduced orphan term reference validation
  - Aligned validation with preview inference logic
- v1.6.2
  - Added orphan validation for term references
  - Added auto-clean for orphaned term amounts
  - Blocked download when validation fails
- v1.6.1
  - Moved contract terms below package cards in preview
  - Added add and delete controls for initial terms
- v1.6.0
  - Package actions and selection improvements
- v1.5.9
...
def render_pricing_page(data: Dict[str, Any]) -> None:
    ...
    if contract_header or contract_initial_terms:
        ...
        if contract_header:
            st.markdown(f'<div class="optum-note">{contract_header}</div>', unsafe_allow_html=True)
